...
class ReactionBuilderModal extends React.PureComponent {
	...
	update = (text = this.state.text) => {
		if (text.length > 0) {
			...
			if (emojiData.emojis.length + this.state.message.reactions.length > 20) {
		...
		for (let i = 0; i < this.state.emojiData.emojis.length; i++) {
			...
			if (
				...
				!this.state.message.reactions?.find(
					(r) => r?.emoji?.name == this.state.message.reactions[emoji] && r?.me
				)
			) {
				addReaction(this.state.message.channel_id, this.state.message.id, {
